...
- [ ] **P-Lab Apple-1 Graphic Card + CodeTank daughterboard** ([graphic](https://p-l4b.github.io/graphic/)): Primary interest — study English/Italian project PDFs, schematic, and BOM for memory-mapped I/O, framebuffer organization, and CodeTank coprocessor role. Cross-check with **apple1-videocard-lib** (below) for software conventions and demos. Possible POM1 work: optional card toggle, RAM window emulation, and ImGui render path (distinct from Uncle Bernie GEN2 at `$2000–$3FFF`).
- [ ] **apple1-videocard-lib** — <https://github.com/nippur72/apple1-videocard-lib>: KickC library + demos for the P-Lab card (TMS9918 at `$CC00` / `$CC01`), VRAM layout, screen modes 1–2, sprites; targets `apple1` vs `apple1_jukebox` (CodeTank / Juke-Box ROM). Use for validation and test ROMs when implementing emulation.
