...
- [ ] **Table-driver les fenêtres hardware** `[M · nice]` — le volet photo est livré (une table `PhotoWindowDef` + un `renderPhotoWindow` générique remplacent 8 paires `ensure<X>Texture()`/`render<X>PhotoWindow()`, −351 l. et −32 membres, et la boucle de teardown a corrigé 3 textures jamais détruites) → `[CHANGELOG.md](CHANGELOG.md)`. Reste ouvert : `MainWindow_HardwareWindows.cpp` (~2454 l.) répète **52 blocs** `ImGui::Begin`/render par carte — les data-driver contre le registry de cartes (livré, `Memory::cardSlots()`). Les 66 flags `show`* du god object `MainWindow_ImGui` (439 membres) veulent devenir un `std::bitset` + enum.
- [ ] **Découpler `CliDispatcher` de `MainWindow_ImGui`** `[S · solid]` — `parseCli()` est une fonction pure sur `argv` mais `CliDispatcher.cpp` `#include` `MainWindow_ImGui.h` pour **deux** accesseurs statiques seulement (`getPresetCount()` / `getPresetName(int)`, 6 sites) ; linker un test unitaire tire donc tout l'UI + ImGui + GLFW. C'est la raison pour laquelle `CliDispatcher` est le seul des 5 trous de tests de juillet 2026 resté non comblé (les 4 autres sont livrés). **Remède** : sortir `kMachinePresets[]` + les deux accesseurs de `MainWindow_Presets.cpp` vers un TU sans UI (`MachinePresets.cpp` ; `MachineConfig` vit déjà dans `MainWindow_Internal.h`), `MainWindow_ImGui::getPresetName` devenant un forwarder. **Débloque aussi l'item `presets.json` externe** ci-dessous, qui a besoin du même découpage.
- [ ] **Scinder la façade `EmulationController`** `[M · nice]` — la fuite d'include vers ImGui est livrée (le contrôleur dépend de `DisplayDevice` + d'un POD `Tms9918DropDiagnostics`, et `TMS9918.h` n'inclut plus `imgui.h`) → `[CHANGELOG.md](CHANGELOG.md)`. Reste l'axe taille : 202 méthodes à scinder selon ses axes — `CpuRunner` (run/step/slice) + `StateManager` (snapshot/rewind) + passthroughs diagnostics.
