...
- [ ] **Extraire le journal `VideoEvents` hors de `Memory`** `[M · solid]` — le reste du chantier beam, et le seul morceau vraiment ouvert. Ce qui est fait (`CHANGELOG.md`) : les deux moteurs partagent une horloge (`BeamClock`), la sérialisation du journal existait déjà (`GEN2VID`, v5+) et un rewind rejoue bien le faisceau — désormais prouvé au pixel par `gen2_journal_snapshot_smoke`, contrôle compris. Reste : `Memory` possède encore les deux moitiés du journal (`gen2RecordingEvents` / `gen2PublishedEvents` et leurs états de trame) alors que `Gen2VideoScanner` en est le propriétaire naturel — un déplacement, sans changement de format. Question ouverte à trancher séparément : le commit progressif par ligne de `TMS9918::advanceCycles` doit-il devenir un `renderUntil(beam)` paresseux ? Ce n'est pas évident — c'est lui qui rend visibles les changements mi-trame.
