...
- [ ] **Partager le journal `VideoEvents` et sérialiser le rejeu beam** `[L · solid]` — **les deux moteurs partagent désormais une seule horloge** (`CHANGELOG.md`) : le GEN2 a adopté `BeamClock` (prouvé exhaustivement sur deux trames entières) et les deux copies inline de la division cycle→ligne du TMS9918 sont supprimées au profit de `pom1::rawLineAt`. Reste trois choses, indépendantes : **(a)** extraire la géométrie et le journal d'événements hors de `Memory` ; **(b)** décider si le commit progressif par ligne de `TMS9918::advanceCycles` doit devenir un `renderUntil(beam)` paresseux — ce n'est PAS fait et ce n'est pas évident : le commit progressif est précisément ce qui rend visibles les changements mi-trame, et le renderer est épinglé par image témoin ; **(c)** sérialiser le journal dans le snapshot pour que le rewind rejoue le faisceau — le seul point qui touche au format, à traiter isolément.
