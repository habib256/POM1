...
- [ ] **Test matriciel headless des 13 presets** `[M · critical]` — **le plus gros levier de test du projet, et l'outil existe déjà** : `--headless` (pas de fenêtre, pas de GL, pas d'ImGui) est implémenté et utilisé par personne en CI. Mesuré le 23 août 2026 : **46 des 109 TU, soit 32 345 lignes = 50 % du code, n'entrent dans aucun binaire de test**. Un test « boot de chaque preset en headless, N frames, sortie 0 » couvre `applyMachineConfig` + la cascade de branchement différé (`kCardEnableDeferFrames`) + les règles Parmigiani — c'est-à-dire exactement le fan-out « ajouter une carte » qui casse. À étendre ensuite aux combinaisons de cartes que le mode strict autorise.
...
- [ ] **Capture du rewind sous `stateMutex`** `[S · solid]` — `runEmulationSlice` sérialise l'état complet (64 Ko + 16 Ko de VRAM TMS + journal GEN2) **4 fois par seconde en tenant le verrou** (`EmulationController.cpp`, `kRewindCaptureIntervalSec = 0.25`). Le TODO ne couvrait que le coût de *seek*. C'est un pic de latence périodique sur le thread UI, de la même famille que le défaut `CassetteDevice` corrigé en août : ni `LockOrder.h` ni TSan ne le voient, seule la lecture du chemin d'appel le trouve. Remède : copier les pages sous le verrou, encoder le delta dehors.
