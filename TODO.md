...
- [ ] **Trois micro-tests TMS9918 échouent sous TSan (et seulement sous TSan)** `[M · solid]` — *(constaté 25 août 2026, après avoir rendu les jobs sanitizer lisibles)* `lib_micro_tests` rend 12/15 sous ThreadSanitizer : `t01_m1_init_sat.s`, `t02_m1_rw_roundtrip.s`, `t05_sprite_attr_terminator.s`. **Ce ne sont pas des dépassements de délai** (6 à 26 s pour un budget de 300 s) mais de vraies assertions, et ils **passent sous ASan+UBSan** — dont l'instrumentation est pourtant plus lourde. La différence entre les deux n'est pas la vitesse mais l'**ordonnancement** : TSan sérialise et réordonne les threads. La piste est donc une dépendance du harnais (ou du chemin VDP) à l'entrelacement thread d'émulation ↔ lecture d'état, du même genre que le seuil chronodépendant corrigé dans `sid_audio_smoke` — sauf qu'ici il faut d'abord établir si la fragilité est dans le test ou dans la synchro beam/CPU du TMS9918, auquel cas ce serait un vrai défaut. Reproduire : `cmake -B build-tsan -DCMAKE_BUILD_TYPE=RelWithDebInfo -DPOM1_SANITIZE=thread` puis `TSAN_OPTIONS=suppressions=tests/tsan.supp ctest -R lib_micro_tests`.
