...
- [ ] **Basculer GitHub Pages sur le déploiement CI, puis dégager le WASM de git** `[S · critical]` — `.github/workflows/pages.yml` est livré et construit le bundle WASM en CI vers la même URL ; il ne fait rien tant que **Settings → Pages → Source** n'est pas passé à *GitHub Actions*. **La bascule doit précéder le premier vrai déploiement** : `actions/deploy-pages` ne peut pas créer de déploiement tant que la source est une branche, donc lancer le workflow avant ne valide que le job `build` (l'artefact est téléchargeable depuis la page du run ; le job `deploy` échoue, et c'est normal). Ordre : (1) basculer le réglage, (2) lancer le workflow, (3) recharger le lien du README, (4) `git rm --cached build-wasm/POM1.{data,wasm,js}` + retirer les trois lignes `!build-wasm/POM1.*` de `.gitignore`. Deux effets de bord : le site ne servira plus que l'artefact (aujourd'hui la racine renvoie le `README.md` rendu par Jekyll ; après, c'est la redirection écrite par le workflow), et **la compilation emsdk du workflow n'a jamais été exécutée** — lire le log du premier run avant de retirer quoi que ce soit de git. Tant que ce n'est pas fait, chaque déploiement web ajoute ~60 Mo à l'historique **définitivement** : 39 révisions de `POM1.data` avaient déjà porté `.git` à 813 Mo. Attention en basculant : Pages ne servira plus que ce que le workflow téléverse, plus le dépôt entier. `build-wasm/cc65/` reste versionné — c'est une dépendance, pas une sortie de build.
- [ ] **Chargement paresseux des assets WASM** `[M · solid]` — `POM1.data` fait encore ~55 Mo après la passe `pic/` (14 → 7 Mo), soit autant à télécharger avant le premier pixel alors que le `.wasm` ne pèse que 3,1 Mo. Ne précharger que le minimum de démarrage (roms, fonts, un preset) et récupérer `software/`, `cassettes/`, `sketchs/` à la demande. C'est le seul canal sans installation, donc celui où le visiteur juge en dix secondes.
...
- [ ] **Filet de crash + bundle de diagnostic** `[M · solid]` — seuls `SIGINT`/`SIGTERM` sont interceptés (pour vider la cassette). Un `SIGSEGV` emporte le journal, la disposition des fenêtres pas encore autosauvegardée et tout le tampon de rewind, et l'utilisateur n'a rien à envoyer. Pour un logiciel maintenu par une personne et distribué à des amateurs sur quatre plateformes, c'est le maillon manquant du support. Gestionnaire de dernier recours écrivant trace d'appels + tampon de journal dans `crash/`, plus *Aide → Signaler un problème* qui assemble journal, snapshot, `ini/` et versions dans un zip. Aucune télémétrie : c'est l'utilisateur qui décide d'envoyer.
- [ ] **`-Werror` sur macOS et Windows** `[S · nice]` — les deux jobs build-only compilent avec `-Wall -Wextra` / `/W4` mais **sans** `-Werror` : l'arbre a été mesuré propre sous GCC uniquement, et AppleClang comme MSVC ont des jeux d'avertissements sensiblement plus larges (conversions signé/non signé, paramètres non référencés). Lire le compte dans les logs du premier run vert, corriger, puis ajouter `-DPOM1_WERROR=ON` à ces deux jobs comme sur Linux.
- [ ] **Étendre le rang des verrous aux mutex des cartes** `[S · nice]` — `LockOrder.h` couvre les trois verrous du cœur. `SID::chipMutex`, `TerminalCard::cardMutex` / `screenshotResultMutex` et les verrous du modem restent hors table ; leur donner un rang (sous `Snapshot`, ou dans une bande dédiée aux périphériques) étendrait la vérification au seul endroit où il reste des verrous non ordonnés.
...
- [ ] **Épingler les modules décrits comme purs** `[S · solid]` — 26 modules de `src/` n'apparaissent dans aucune cible de test, et presque tous sont de l'UI. Deux ne le sont pas et sont déjà testables tels quels : `Pom1BenchCc65.cpp` (« des fonctions sur des chaînes et des chemins, sans MainWindow, sans EmulationController, sans ImGui ») et la table `ConflictRule` de `MainWindow_SiliconStrict.cpp`, qui est de la politique de bus pure. Règle à retenir de la passe précédente : **quand un module est décrit comme pur dans la doc, il devrait avoir un test qui le prouve.**
