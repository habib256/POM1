...
- [ ] **Basculer GitHub Pages sur le déploiement CI, puis dégager le WASM de git** `[S · critical]` — `.github/workflows/pages.yml` est livré et construit le bundle WASM en CI vers la même URL ; il ne fait rien tant que **Settings → Pages → Source** n'est pas passé à *GitHub Actions*. **La bascule doit précéder le premier vrai déploiement** : `actions/deploy-pages` ne peut pas créer de déploiement tant que la source est une branche, donc lancer le workflow avant ne valide que le job `build` (l'artefact est téléchargeable depuis la page du run ; le job `deploy` échoue, et c'est normal). **L'étape (4) est LIVRÉE** (mesuré août 2026 : `git log --all -- build-wasm/POM1.data` → 0 commit, `.git` retombé de 813 Mo à 245 Mo, et les trois lignes `!build-wasm/POM1.*` ont disparu de `.gitignore` — seul `!build-wasm/POM1.html` reste, volontairement). Reste donc : (1) basculer le réglage, (2) lancer le workflow, (3) recharger le lien du README — un réglage web et une lecture de log, plus un chantier. Deux effets de bord : le site ne servira plus que l'artefact (aujourd'hui la racine renvoie le `README.md` rendu par Jekyll ; après, c'est la redirection écrite par le workflow), et **la compilation emsdk du workflow n'a jamais été exécutée** — lire le log du premier run avant de retirer quoi que ce soit de git. Tant que ce n'est pas fait, chaque déploiement web ajoute ~60 Mo à l'historique **définitivement** : 39 révisions de `POM1.data` avaient déjà porté `.git` à 813 Mo. Attention en basculant : Pages ne servira plus que ce que le workflow téléverse, plus le dépôt entier. `build-wasm/cc65/` reste versionné — c'est une dépendance, pas une sortie de build.
- [ ] **Chargement paresseux des assets WASM** `[M · solid]` — `POM1.data` fait **62 Mo mesurés** (août 2026, pas les ~55 annoncés) à télécharger avant le premier pixel, alors que le `.wasm` ne pèse que 3,1 Mo. C'est le seul canal sans installation, donc celui où le visiteur juge en dix secondes. Décomposition mesurée : **`cfcard/cfcard.po` 32 Mo (52 %)**, `pic/` 7,0, `sketchs/` 5,1, `cassettes/` 3,7, `dev/` 2,2, `software/` 1,6, `sdcard/` 1,6, `fonts/` 1,2, `roms/` 0,6.
  - **`cfcard.po` à lui seul vaut 52 % du gain** et ne sert que si la CFFA1 est branchée. Attention : ce n'est PAS un simple `--preload-file` en moins — `Memory`'s constructeur ouvre l'image inconditionnellement au démarrage (`cffa1->openDiskImage`, sonde `cfcard/`, `../cfcard/`, `../../cfcard/`), donc la voie paresseuse demande un fetch asynchrone plus un état « image en cours de chargement » côté carte, et n'est vérifiable qu'en navigateur. Estimé S dans un premier temps, **c'est un M** ; le reste (30 Mo) en est un second.
