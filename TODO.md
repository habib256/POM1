...
- [ ] **Le callback audio décode sous verrou pendant que le thread d'émulation attend** `[M · solid]` — *(audit général 24 août 2026)* même famille que le défaut `CassetteDevice` d'août — une DURÉE de détention, invisible pour `LockOrder.h` comme pour TSan — mais sur le chemin de **lecture**, resté en place. `fillAudioBuffer` prend `audioStreamMutex` **puis** appelle `ma_decoder_read_pcm_frames`, dont les recharges font un `fread` disque, sur le thread temps-réel. En face, `SnapshotPublisher::publish` lit `getPlaybackPositionSeconds()`/`getPlaybackTotalSeconds()` — donc réclame ce même verrou — **en tenant `stateMutex`**. Avec une cassette en mode flux montée, chaque tranche d'émulation (~170/s) peut donc bloquer sur une lecture disque en tenant le verrou d'état, et tout appel UI attend derrière. Remède : sortir la position lue du verrou (atomique publiée par le décodeur) ou ne pas la publier à chaque trame. Note : `audioMutex`/`audioStreamMutex` sont des `std::mutex` nus, hors de la table des rangs — voir l'item « étendre le rang des verrous aux mutex des cartes ».
- [ ] **Synthèse du clic de cassette sous `audioMutex`** `[S · nice]` — *(audit général 24 août 2026)* `fireClickIfModeChanged` alloue ~13 ko et enchaîne ~3400 itérations de `sin`/`exp` **en tenant `audioMutex`**, que `fillAudioBuffer` réclame à chaque période ; depuis `loadAudioStream` les deux verrous du deck sont pris. Micro-coupure audio à chaque transition du deck (insertion, éjection, programme↔flux) — négligeable sur bureau, audible sur la borne Pi. Contredit la règle écrite dans `AudioDevice.h` (« the audio callback must not allocate and must not wait on slow work »). Remède : pré-calculer le clic une fois (le taux ne change pas) et n'échanger qu'un pointeur sous le verrou.
