...
KBDD            :=     $D010          ; PIA.A keyboard input
KBDCR           :=     $D011          ; PIA.A keyboard control register
DSP             :=     $D012          ; PIA.B display output register
DSPCR           :=     $D013          ; PIA.B display control register

; KBDD b7..b0 are inputs, b6..b0 is ASCII input, b7 is constant high
;     Programmed to respond to low to high KBDD strobe
; DSP b6..b0 are outputs, b7 is input
;     CB2 goes low when data is written, returns high when CB1 goes high
; Interrupts are enabled, though not used. KBDD can be jumpered to IRQ,
...
WBS              :=     $DF            ; Backspace key, arrow left key
WCR              :=     $8D            ; Carriage Return
ESC             :=     $9B            ; ESC key
...
RESET:          CLD                  ;   Clear decimal arithmetic mode
                ...
                LDA     #%10100111   ;   KBDD and DSP control register mask
...
NEXTITEM:       LDA     IN,Y         ;   Get character
                ...
                CMP     #'R'+$80
                BEQ     RUNPROG      ;   Run the program! Forget the rest
...
RUNPROG:        JMP     (XAML)       ;   Run user's program
