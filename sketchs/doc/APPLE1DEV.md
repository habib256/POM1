...
| [`doc/README.md`](../../doc/README.md) | **Index of all docs** (guides, card references, CLI, architecture). |

**Contents:** sections **§1–§12** numbered below (skim the titles).
...
| 1976 SWTPC graphics | asm + GT-6144 | 64×96 mono | `sketchs/apple1/gt6144_demo_hello/gt6144.cfg` |
...
```bash
ca65 -o build/MyProg.o sketchs/<profile>/<name>/MyProg.asm
...
The text config (`apple1_4k.cfg`) reserves **`$0000-$0022` ZP** (35 bytes); Wozmon + ACI claim the rest. Other configs reserve more (`apple1_gen2.cfg` = `$0000-$003F`, the C configs claim the full ZP page). `Memory::loadHexDump()` accepts canonical Wozmon dumps: `AAAA: HH HH …` lines, optional `:` separator, `R` suffix at EOF for auto-run, inline `//` `#` `;` comments stripped (otherwise mnemonic letters like `LDA`/`DEX` would parse as data). File extensions routed to this parser: `.txt` `.hex` `.apl` `.mon` `.tur` (`src/HexDumpFile.h`); `.apl` is Uncle Bernie's canonical name for the same content. A **`.TUR`** (TurboType) is detected by its lone `T` line and gets a **line-structured** parse — its blocks put the address on its own line and prefix each 16-byte data line with `:`, so the legacy joined-line rules would mis-slice them. POM1 injects RAM directly, so the file's autotyped serial receiver is never executed; the trailing `015ER`-style run line points at the file's own CRC-CCITT checker, which validates the injected image and jumps to the program itself.
...
Reference: `sketchs/apple1/sid_piano/Claudio_PARMIGIANI_SID_PIANO_AZERTY.asm`. **C64 `.sid` conversion**: `tools/sid2apple1.py` rewrites `$D400` → `$C800`, neutralises CIA/VIC, emits `.bin` for `$0280`. Source tunes at [HVSC](https://www.exotica.org.uk/wiki/High_Voltage_SID_Collection).
...
24-reg broadcast on 100-cycle period with PORTB STROBE. Regs 0-5 = RTC (H/M/S/D/M/Y), reg 6 = DS3231 die temp, ADC + digital in/out follow. Demo: `sketchs/apple1/io_rtc_clock/RtcClock.asm`.
...
Complete verb table (all phases): [`CLI.md`](../../doc/CLI.md).
...
| SID direct register play | `sketchs/apple1/sid_piano/Claudio_PARMIGIANI_SID_PIANO_AZERTY.asm` |
| SID from C64 conversion | `python3 tools/sid2apple1.py Music.sid` |
| RTC / sensors | `sketchs/apple1/io_rtc_clock/RtcClock.asm` |
