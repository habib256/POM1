...
3. **GT-6144**: the framebuffer lives on 6× Intel 2102 SRAM (no Apple-1
   RAM), so `apple1_4k.cfg` works. `sketchs/apple1/gt6144_demo_hello/gt6144.cfg` is a
...
5. **Dual-bank / large programs**: project-local `.cfg` that splits low
   ...
   `sketchs/apple1/game_chess/apple1_chess_text.cfg` (text) and
   `sketchs/gen2/game_sokoban/apple1_sok_hgr.cfg` (HGR) for the canonical
...
Replaces the ~10 near-identical `emit_<PROJECT>_txt.py` scripts that lived
under `sketchs/` (multi-file projects still use them). Each such project keeps a 5-line `emit_<PROJECT>_txt.py`
...
complex multi-file projects still under `sketchs/`, ship with this
convention (their pre-migration `emit_*_txt.py` scripts of 60+ lines
compressed to ~23 lines apiece). Discoverable via
`find sketchs -name 'emit_*_txt.py' 2>/dev/null`.

## Makefile.common — shared Makefile fragment

`Makefile.common` absorbs the `ca65` / `ld65` / `emit_*_txt.py`
boilerplate that every multi-file project Makefile under
every multi-file project was reinventing. A typical project Makefile is now five
...
**Multi-binary or multi-object projects** (`gen2/game_rogue_x2`,
`gen2/demo_sprite_animals`, `gen2/game_maze3d`, `tms9918/tool_tmsload`,
`tms9918/tool_diapo`, and the two `game_chess` dual-bank variants) keep
their own custom Makefiles. The shared fragment covers the single-binary
case only — today `sketchs/apple1/_template/` is its one consumer.
